...
async function init(): Promise<void> {
  console.log("\n" + c.music("╔══════════════════════════════╗"));
  console.log(
    c.music("║") + c.bold("  🎵 Meet Music Bot v1.0      ") + c.music("║"),
  );
  console.log(c.music("╚══════════════════════════════╝"));
  ...
  if (!okSetup) {
    err(
      "Falha ao criar microfone virtual. Verifique se o PulseAudio/PipeWire está instalado.",
    );
    process.exit(1);
  }
...
const COMMANDS: Record<string, CommandHandler> = {
  async join(args) {
    if (!args)
      return err("Uso: !join <url ou código do Meet> [nome visitante]");
    const urlMatch = args.match(
      /^(https?:\/\/\S+|[a-z]{3}-[a-z]{4}-[a-z]{3})\s*(.*)?$/i,
    );
    const url = urlMatch ? urlMatch[1] : args;
    const name =
      urlMatch && urlMatch[2] && urlMatch[2].trim()
        ? urlMatch[2].trim()
        : guestName;
    ...
    if (joined) ok("Entrei na sala! Microfone virtual ativo.");
      ...
      warn(
        'Não consegui clicar em "Entrar". Tente manualmente na janela do navegador.',
      );
  },
  ...
  async volume(args) {
    ...
    if (Number.isNaN(v) || v < 0 || v > 200)
      return err("Volume deve ser entre 0 e 200.");
  ...
  async queue_cmd() {
    ...
    log(c.bold("\n  Fila de músicas:"));
    if (queue.current)
      log(c.music("  ▶ ") + c.bold(queue.current.title) + c.dim(" (tocando)"));
    items.forEach((it, i) =>
      log(c.dim(`  ${i + 1}. `) + it.title + c.dim(` [${it.duration}]`)),
    );
    log("");
  },
  ...
  async help() {
    const cmds: [string, string][] = [
      [
        "!join <url> [nome]",
        "Entrar em uma sala (nome opcional para visitante)",
      ],
    ...
    cmds.forEach(([cmd, desc]) =>
      log("  " + c.music(cmd.padEnd(26)) + c.dim(desc)),
    );
    log("");
  },
